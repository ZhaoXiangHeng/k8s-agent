...
项目既要满足"基于 MCP 的 K8S AI 运维助手"的核心需求，也要体现接近企业级平台的设计能力：多角色、权限隔离、LLM 管理、审计、安全边界、Helm 部署和离线镜像交付。

## 核心价值

- 用自然语言降低 Kubernetes 巡检和常见运维操作门槛
- 用 Keycloak 和 Kubernetes RBAC 建立清晰的身份与权限边界
- 用 Traefik API 网关统一外部流量入口，按路径路由到前端、API 和 Keycloak
- 用 Backend 统一管控 LLM 工具调用，避免 LLM 直接接触集群凭据
- 用 Agent Server 承接 Eino ReAct agent loop，让 LLM 编排和业务 API 解耦
...
- Traefik API 网关统一外部入口，gRPC/SSE 走内部 ClusterIP
- 所有关键操作写审计日志
...
- 技术评审能通过架构图看懂 UI、Traefik、Backend、Agent Server、MCP Server、LLM、K8s API 的调用链
