...
- **Agent 与多轮上下文**：Backend 保存 Chat Session/Message，负责读取、裁剪、脱敏多轮历史；Agent Server 无状态，不持久化会话历史；Backend 与 Agent Server 通过 proto 生成的 gRPC 契约通信
- **文档**：全部使用中文
- **日志**：程序日志使用英文结构化日志（logrus JSON），API Key/token/Secret 不得出现在日志和审计中
- **部署**：支持 Helm 部署，支持本地 tar 镜像包和 registry 镜像，内置 Traefik 作为 API 网关
- **存储**：支持 PostgreSQL 和 Redis
- **认证**：支持 Keycloak 认证
- **权限**：操作员权限仅限 namespace 级

## 验收标准

- 管理员可以完成"创建操作员 → 分配权限 → 绑定模型"流程
- 操作员只能看到和操作授权范围内资源
- 未授权工具调用会被 MCP Server 拒绝，并进入审计链路
- 即使 Backend 校验有缺陷，Kubernetes RBAC 也会拒绝越权访问
